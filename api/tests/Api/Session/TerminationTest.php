...
class TerminationTest extends WebTestCase
{
    ...
    private function createSession(string $ip, string $owner): Session
    {
        $license = $this->createLicense($owner);
        $location = $this->createLocation($ip);
    ...
    {
        $session = $this->createSession("127.0.0.1", "ownertest");

        $requestBody = $this->createValidRequestBody($session->getLicense()->getUuid(), $session->getLocation()->getIpAddr());
        ...
        $this->assertValidResponse(
            ...
            iv: $requestBody["iv"],
            expectedCode: 200,
            expectedMessage: "Session terminated."
        );
    }
}